...
const Projects = () => {
  ...
  // Добавляем состояние для просмотра фотографий
  const [isViewerOpen, setIsViewerOpen] = useState(false);

  // Эффект для управления шапкой
  useEffect(() => {
  ...
  const handleImageClick = (project: Project) => {
    ...
    // Скрываем шапку при открытии фото
    const header = document.querySelector('header');
    if (header) {
  ...
  return (
    <>
      <SEO 
        title="Portfolio de Proyectos | Plakor Divisiones"
        description="Explore nuestros proyectos de construcción y reformas en Tarragona. Galería de trabajos realizados en viviendas y locales comerciales."
        keywords="proyectos construcción, reformas realizadas, portfolio obras, Tarragona"
      />
      <div className="py-20 overflow-x-hidden">
        <section ref={ref} className="container mx-auto px-4">
          ...
          {/* При открытии просмотрщика фотографий */}
          <div 
            className={`fixed inset-0 bg-black z-50 transition-opacity duration-300 ${
              isViewerOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
            }`}
          >
